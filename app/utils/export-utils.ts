...
import { formatDateForExport, formatDateWithLocale, SupportedLocale } from './date-utils';
...
// Lazy load XLSX
const loadXLSX = async (): Promise<XLSXType> => {
...
// Export to Excel
export const exportToXLS = async (data: ExportData): Promise<void> => {
  ...
  for (let col = headerRange.s.c; col <= headerRange.e.c; col++) {
    ...
    worksheet[cellAddress].s = {
      ...
      fill: { fgColor: { rgb: '479AF3' } },
      alignment: { horizontal: 'center' },
    };
  }

  // Add worksheet to workbook
  const sheetName = translations['Hospitals'] ?? 'Hospitals';
  ...
  const metadataData = [
    ...
    ['', ''],
    [translations['Status Distribution'] ?? 'Status Distribution', ''],
    [
      translations['Deployed'] ?? 'Deployed',
      hospitals.filter(h => h.status === 'Deployed').length.toString(),
    ],
    [
  ...
  metadataWorksheet['!cols'] = [{ wch: 25 }, { wch: 15 }];

  const metadataSheetName = translations['Metadata'] ?? 'Metadata';
  XLSX.utils.book_append_sheet(workbook, metadataWorksheet, metadataSheetName);

  // Save the Excel file with internationalized filename
  const filenamePrefix = locale === 'fr' ? 'galeon-hopitaux' : 'galeon-hospitals';
