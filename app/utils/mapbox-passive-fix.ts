...
export function applyMapboxPassiveEventsFix(): void {
  ...
  (
    window as unknown as { _originalAddEventListener: typeof originalAddEventListener }
  )._originalAddEventListener = originalAddEventListener;
  ...
  EventTarget.prototype.addEventListener = function (
    ...
    // Liste des événements qui devraient être passifs dans la plupart des cas
    const passiveEvents = ['mousewheel'];

    // Ces événements peuvent nécessiter preventDefault() dans certains cas
    // donc on ne les force pas en passif
    const nonPassiveEvents = ['touchstart', 'touchmove', 'touchend', 'wheel'];

    // Si c'est un événement qui devrait être passif
    if (passiveEvents.includes(type)) {
      // Conversion des options en objet si nécessaire
      const optionsObject =
        typeof options === 'object' ? options : options === undefined ? {} : { capture: options };
...
export function removeMapboxPassiveEventsFix(): void {
  ...
  const windowWithOriginal = window as unknown as {
    _originalAddEventListener?: typeof EventTarget.prototype.addEventListener;
  };
  const originalAddEventListener = windowWithOriginal._originalAddEventListener;
  if (originalAddEventListener) {
    EventTarget.prototype.addEventListener = originalAddEventListener;
    delete windowWithOriginal._originalAddEventListener;
  }
}
