...
const ActionBar: React.FC<ActionBarProps> = ({ className = '' }) => {
  ...
  // Fermer les menus lorsque l'utilisateur clique en dehors ou appuie sur Escape
  useEffect(() => {
  ...
  const exportHospitalData = useCallback(
    (format: 'pdf' | 'csv' | 'json') => {
      try {
        ...
        logger.debug(
          `Starting ${format} export... (${rateLimitResult.remaining} exports remaining)`
        );
        ...
        const successMessage =
          rateLimitResult.remaining !== undefined && rateLimitResult.remaining > 0
            ? _(
                `${format.toUpperCase()} export completed successfully! (${rateLimitResult.remaining} exports remaining)`
              )
  ...
  return (
    <div className={`${className}`}>
      ...
      <div
        ref={actionBarRef}
        className="action-bar-container relative flex flex-row items-center justify-center gap-2 px-2 w-[130px] h-[52px] md:w-[140px] md:h-[55px] bg-[rgba(217,217,217,0.05)] border-2 border-[rgba(71,154,243,0.3)] backdrop-blur-[17.5px] rounded-xl"
        role="toolbar"
      ...
      >
        ...
        <button
          ref={exportBtnRef}
          className="flex items-center justify-center bg-transparent border-0 action-button w-[40px] h-[40px] min-w-[40px] min-h-[40px] p-0 m-0"
        ...
        <PopupMenu
          ...
          anchorRef={actionBarRef}
          id="export-menu"
        >
          <div className="text-[#A1CBF9] text-xs sm:text-sm mb-4">{_('Choose export format')}</div>
          <div className="flex justify-between items-center gap-3 py-3">
            ...
            >
              ...
              <span
                className="text-[#A1CBF9] text-xs md:text-sm lg:text-base h-6 md:h-8 lg:h-10 flex items-center justify-center"
        ...
        {/* Séparateur */}
        <div className="h-[24px] w-0 border-l border-[#A1CBF9]"></div>
