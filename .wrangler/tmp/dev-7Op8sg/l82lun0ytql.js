...
// .wrangler/tmp/bundle-3v05Qx/checked-fetch.js
...
// .wrangler/tmp/pages-S6550g/bundledWorker-0.7435084919054984.mjs
...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (isDev) {
        const isFromNextJs = request.headers.get("x-forwarded-from-next") === "true";
        if (!isFromNextJs) {
          try {
            console.log(`Proxying request to Next.js: ${pathname}`);
            const nextUrl = new URL(request.url);
            nextUrl.port = "3000";
            nextUrl.hostname = "localhost";
            const headers = new Headers(request.headers);
            headers.set("x-forwarded-from-next", "true");
            ...
            if (nextResponse.status === 200) {
              console.log(`Next.js responded with status: ${nextResponse.status}`);
              return nextResponse;
            } else {
              console.log(`Next.js failed with status: ${nextResponse.status}, falling back to worker`);
            }
          } catch (e) {
            console.error(`Error proxying to Next.js: ${e}`);
            console.log("Falling back to worker-served content");
          }
        } else {
          console.log("Request already forwarded from Next.js, serving from worker to avoid loops");
        }
      }
...
<body>
  <div class="container">
    ...
    <p>En mode d\xE9veloppement, veuillez acc\xE9der directement \xE0 <a href="http://localhost:3000" style="color: blue; background: none; padding: 0;">http://localhost:3000</a></p>
...
// .wrangler/tmp/pages-S6550g/l82lun0ytql.js
...
// .wrangler/tmp/bundle-3v05Qx/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3v05Qx/middleware-loader.entry.ts
...
//# sourceMappingURL=l82lun0ytql.js.map
